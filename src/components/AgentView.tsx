...
const AgentView: React.FC = () => {
  ...
  return (
    <div className="flex h-full bg-nexus-950 transition-colors duration-300">
        ...
        <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full border-r border-nexus-border/0 md:border-r md:border-nexus-border">
            <div className="flex-1 overflow-y-auto p-4 space-y-6" ref={scrollRef}>
                        ...
                        <div className={`flex flex-col max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                            <div className={`p-4 rounded-2xl ${
                                ...
                                <div className="prose prose-invert prose-sm whitespace-pre-wrap">
                                    <span className="text-nexus-text-primary">{msg.text}</span>
                                </div>
                            </div>
                            ...
                            {msg.role === 'model' && (
                                <div className="mt-2 flex items-center space-x-2">
                                    <span className="text-[10px] uppercase text-nexus-text-secondary font-bold tracking-wider">{t('agent.sources')}:</span>
                                    <span className="text-xs text-blue-400 hover:underline cursor-pointer bg-blue-400/10 px-2 py-0.5 rounded border border-blue-400/20">architecture_v2.md</span>
                                    <span className="text-xs text-blue-400 hover:underline cursor-pointer bg-blue-400/10 px-2 py-0.5 rounded border border-blue-400/20">optimization.txt</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {isTyping && (
                    <div className="flex gap-4">
                        ...
                        <div className="flex items-center text-nexus-text-secondary text-sm italic">
            ...
            <div className="p-4 border-t border-nexus-border bg-nexus-950">
                <div className="relative">
                    <input
                        className="w-full bg-nexus-900 border border-nexus-border rounded-xl pl-4 pr-12 py-3 text-nexus-text-primary focus:ring-1 focus:ring-nexus-accent focus:border-nexus-accent transition-all outline-none"
        ...
        <div className="hidden lg:flex w-[400px] bg-nexus-900 flex-col border-l border-nexus-border transition-colors duration-300">
            <div className="p-4 border-b border-nexus-border flex items-center justify-between">
                <div className="flex items-center gap-2 text-nexus-text-primary font-semibold">
            ...
            <div className="flex-1 p-6 flex flex-col items-center justify-center text-center">
                 ...
                 <h3 className="text-lg font-medium text-nexus-text-primary mb-2">{t('agent.synthesisTitle')}</h3>
                 <p className="text-sm text-nexus-text-secondary px-8">
                     {t('agent.synthesisDesc')}
                 </p>
                 
                 <div className="mt-8 w-full bg-nexus-950 rounded border border-nexus-border p-4 text-left">
                    <div className="text-xs font-mono text-nexus-text-secondary mb-2 border-b border-nexus-border pb-2">{t('agent.draft')}: optimization_plan.md</div>
