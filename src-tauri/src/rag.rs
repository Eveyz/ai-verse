...
impl RagSystem {
    ...
    async fn get_table(&self, name: &str) -> Result<Table> {
        ...
        if table_names.contains(&name.to_string()) {
            ...
            Ok(self.db.create_table(name, RecordBatchIterator::new(vec![], schema.clone()))
                .execute().await?)
        }
    }
    ...
    pub async fn add_document(&mut self, content: &str) -> Result<()> {
        ...
        let ids: Vec<String> = (0..total_chunks).map(|_| ulid::Ulid::new().to_string()).collect();
        ...
        let batch = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(StringArray::from(chunks.into_iter().map(String::from).collect::<Vec<String>>())),
                vector_array,
            ],
        )?;
    ...
    pub async fn search(&mut self, query: &str, limit: usize) -> Result<Vec<String>> {
        ...
        for batch in batches {
            let text_col = batch.column_by_name("text").ok_or(anyhow::anyhow!("Col missing"))?;
